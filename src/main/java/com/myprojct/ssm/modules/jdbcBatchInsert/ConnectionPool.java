...
/*
 * 数据库连接池类 这个例子是根据POSTGRESQL数据库写的， 请用的时候根据实际的数据库调整。 调用方法如下： ① ConnectionPool connPool = new
 * ConnectionPool("com.microsoft.jdbc.sqlserver.SQLServerDriver"
 * ,"jdbc:microsoft:sqlserver://localhost:1433;DatabaseName=MyDataForTest" ,"Username"
 * ,"Password"); ② connPool .createPool(); Connection conn = connPool .getConnection();
 * connPool.returnConnection(conn); connPool.refreshConnections(); connPool.closeConnectionPool();
 */
